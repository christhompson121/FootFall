...
router.get('/', function(req, res, next) {
  res.json({
    "message": "Welcome to the Facebook Event Search service!"
  });
});
...
router.get('/events', function(req, res, next) {
  ...
  if (!req.query.lat || !req.query.lng || !req.query.distance) {
    res.status(500).send({
      error: "Please specify the lat, lng, distance query parameters"
    });
  } else {
    ...
    var idLimit = 50, //FB only allows 50 ids per /?ids= call
      ...
      venuesWithEvents = 0,
      eventsCount = 0,
      placeUrl = "https://graph.facebook.com/v2.5/search?type=place&q=*&center=" + req.query.lat + "," + req.query.lng + "&distance=" + req.query.distance + "&limit=100000&fields=id&access_token=" + ACCESS_TOKEN;
    ...
    rp.get(placeUrl).then(function(responseBody) {
        ...
        ids.forEach(function(idArray, index, arr) {
          urls.push(rp.get("https://graph.facebook.com/v2.5/?ids=" + idArray.join(",") + "&fields=id,name,cover.fields(id,source),picture.type(large),location,events.fields(id,name,cover.fields(id,source),picture.type(large),description,start_time,attending_count,declined_count,maybe_count,noreply_count).since(" + currentTimestamp + ")&access_token=" + ACCESS_TOKEN));
        });
      ...
      .then(function(results) {
          ...
          Object.getOwnPropertyNames(resObj).forEach(function(venueId, index, array) {
            ...
            if (venue.events && venue.events.data.length > 0) {
              ...
              venue.events.data.forEach(function(event, index, array) {
                ...
                /*console.log("Checking hours till event");
              
                console.log("hoursTillEvent specified: " + req.query.hoursTillEvent);
                if (req.query.hoursTillEvent < eventResultObj.eventTimeFromNow / 60 / 60) {
                  events.push(eventResultObj);
                  eventsCount++;
                  console.log(eventResultObj.eventTimeFromNow);
                  console.log(eventResultObj.eventTimeFromNow /60 /60);
                  console.log("is " + req.query.hoursTillEvent + " < " + (eventResultObj.eventTimeFromNow / 60 / 60).toString());
                }*/

                if (req.query.startCheckDate && req.query.endCheckDate) {
                  var startCheckDate = new Date(req.query.startCheckDate);
                  var endCheckDate = new Date(req.query.endCheckDate);
                  console.log("In date filtering");
                  // var startCheckDate = new Date("2016-03-12T13:00");
                  // var endCheckDate = new Date("2016-03-22T13:00");
                  var eventStarttimeP = new Date(eventResultObj.eventStarttime)
                  if (startCheckDate < eventStarttimeP && eventStarttimeP < endCheckDate) {
                    events.push(eventResultObj);
                    eventsCount++;
                  }
                } else {
        ...
        res.send({
          ...
          metadata: {
            venues: venuesCount,
            venuesWithEvents: venuesWithEvents,
            events: eventsCount
          }
        });
